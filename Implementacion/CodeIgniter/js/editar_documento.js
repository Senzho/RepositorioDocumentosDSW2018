$(document).ready(function(){
	var edicion = false;
	if($("#texto_documento").val() != ""){
		var textoPagina = $("#texto_documento").val();
		var patron = "!----!";
		textoPagina = textoPagina.split('!----!').join('</p>');
		console.log(textoPagina);
		CKEDITOR.instances['editor'].setData(textoPagina);
		edicion = true;
	}
	...
	$("#modificar_documento").on("submit", function(event){
		...
		if($('#nombreDocumentoCrear').val()==""){
			...
			document.getElementById('modificar_documento').submit();
		}
	});
